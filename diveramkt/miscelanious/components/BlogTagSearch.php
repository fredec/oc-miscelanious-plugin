...
class BlogTagSearch extends ComponentBase
{
    ...
    public function onLoadPage($page = false)
    {
        ...
        if(empty($this->tag)) {
            ...
            // $this->posts = \RainLab\Blog\Models\Post::orderBy('published_at','desc')->get();

            $this->posts = Post::whereHas('tags', function($tag) {
                $tag->where('name', $this->property('tag'))->orWhere('slug', $this->property('tag'));
            })->orderBy('published_at','desc')->isPublished();

            if(isset($_GET['search'])){
                $searchableFields = ['title', 'slug', 'excerpt', 'content'];
                $search = trim($_GET['search']);
                if (strlen($search)) $this->posts=$this->posts->searchWhere($search, $searchableFields);
            }

            if($this->property('exceptPost')){
                $except=$this->property('exceptPost');
                $this->posts=$this->posts->where(function($query) use($except){
                    if(is_numeric($except)) $query->where('id','!=',$except);
                    else $query->where('slug','!=',$except);
                });
            }
            $this->posts=$this->posts->paginate($this->property('resultsPerPage'), $this->property('page'));
